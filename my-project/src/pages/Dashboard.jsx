...
const Dashboard = () => {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    (data.history || []).forEach(msg => {
                        ...
                        const baseId = Number(msg.id) || Date.now();
                        grouped[cid].push(
                            {
                                id: `user-${baseId}-${Math.random()}`,
                                role: 'user',
                                content: String(msg.message || '')
                            },
                            {
                                id: `bot-${baseId}-${Math.random()}`,
                                role: 'assistant',
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            const bot = {
                id: `bot-fallback-${Date.now()}-${Math.random()}`,
                role: 'assistant',
            ...
            if (!activeChatId) {
                ...
                setAllMessages(prev => {
                    const tempMsgs = [...(prev['temp'] || []), userMsg, bot];
